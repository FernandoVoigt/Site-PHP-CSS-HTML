   ...
   <div id='cssmenu'>
        ...
        <div id='cssmenu2'>
			...
			<div class="conteudo2">
				...
				$i=0;
				$check= count($_POST["checkbox"]);
				
				echo "<center>Os seguintes produtos foram deletados:</br></br></ceter>";
						 				
				for($i = 0; $i < $check; $i++) 
				{
						$codproddel[$i]=$_POST["checkbox"][$i];
						$query1= "select * from ".$table." where cod=".$codproddel[$i].";";
						$exe1= mysql_query($query1);
						while($retorno = mysql_fetch_array($exe1))
						{
							echo  ($i+1) .". " . $retorno['modelo'] . "</br>";
						}
						$query="delete from " . $table . " where cod=".$codproddel[$i].";" ;
						mysql_query($query);
				...
				{
					echo "<center>Nenhum produto foi selecionado</center>";
				}
				?> 
			</div>
    	 <div class="rodape">
